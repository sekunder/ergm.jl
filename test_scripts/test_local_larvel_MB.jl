...
for chi in ["0.25"]
    ...
    target_stats = [sum(small_larvalMB)/2, tr(small_scaffold^3)/n] #could try scale this better with an n^2 -- meh, this didnt really work
    ...
    burn_in = 10 * n^2
    sample_interval =  n^2 # increasing this by a factor of 2 - not really a notable difference here
    iterations = 50    #I think we should take this into the 500 realm - WIP
    learning_rate = 1.0 
    ...
    densities = cat([[sum(A) / (n * (n-1))  tr(A^3) / (n * (n-1) * (n-2))] for A in As]..., dims=1) #same thing for here perhaps
    ...
    display(p) # Display the density plot

    trajectory_fig = plot(thetas[:,1], thetas[:,2],
                        title="Theta trajectory", label=L"\theta_t",
                        xlabel=L"\theta_1", ylabel=L"\theta_2")

    display(trajectory_fig) # Display the trajectory plot

    log_message("Plots displayed.", verbose)
end
